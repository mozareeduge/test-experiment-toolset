...
This skill reads the `mozare-write` skill's own files as siblings (typically
`~/.claude/skills/mozare-finalize/` next to `~/.claude/skills/mozare-write/`
for a user-level install, or the equivalent under one repo's `.claude/` for
a project-level one). Every bare filename below (`protocol.md`,
`structural-tells.md`, `banned-hard.txt`, `banned-review.txt`) refers to
that sibling directory — locate it once with
...
7. **Write the cleared copy** — only on an outcome that actually clears.
   **Where it goes depends on what invoked this skill is running inside:**
   ...
   - Otherwise (invoked from an arbitrary local folder with no enclosing
     ...
     unrelated projects): write to `~/.claude/mozare-finalize/exports/<slug>--<date>--<n>.md`
...
8. **Write the audit record unconditionally** — this is the last action of
   *every* finalize run, including one that stops at step 2 or fails at
   ...
   a repo, or `~/.claude/mozare-finalize/audits/<date>--mozare-finalize--<slug>--<n>.md`
